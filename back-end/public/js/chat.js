...
async function getMessages() {
    try {
        const messages_arr = JSON.parse(localStorage.getItem('messages'));
        let last_message_id = messages_arr[messages_arr.length-1].id;
        const result = await axios.get(`http://localhost:4000/get-messages?id=${last_message_id}`, { headers: { Authorization: token } });
        localStorage.setItem('messages', JSON.stringify(result.data));
